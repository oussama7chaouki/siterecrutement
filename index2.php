<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=
    , initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="index.css">
    <script defer src="hicham.js"></script>

</head>
<body>
<header>

 <a href="#" class="logo">DREAMJOB</a>
 <nav class="nav">

 <a href="#login">Login</a>
 <a href="#signup">Sign Up</a>
 
 </nav>

</header>

<section class="main">

<div class="content">
<h2 id="oussamach">find a <span id="och" class="grish">job </span> that<br><span class="grish">matches</span> your <span id="ouch">  passion</span></h2>
<h3>Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque praesentium aut ex incidunt porro voluptas repellat.</h3>

<div class="row">

    <button onclick="window.location.href='login.php';" class="card"><a  href="login.php">SIGN IN</a></button>
        

    <button onclick="window.location.href='login.php';"  class="card"><a href="login.php">SIGN UP</a></button>

</div>


</div>
</section>
<!-- <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
    <path fill="#C4F2FF" fill-opacity="1" d="M0,160L60,165.3C120,171,240,181,360,197.3C480,213,600,235,720,229.3C840,224,960,192,1080,165.3C1200,139,1320,117,1380,106.7L1440,96L1440,0L1380,0C1320,0,1200,0,1080,0C960,0,840,0,720,0C600,0,480,0,360,0C240,0,120,0,60,0L0,0Z"></path>
  </svg> -->
<!----------2end section---->
<section class="main2">


    <div>
       

        </div>

        <h2>why we are different?</h2>
        <h3>make the choosing process<br>
            faster  and easier for both side<br>
            by calculating the candidate score <br>
           and parsing data from the cv doc directly</h3><br>
           
           <img src="hicham.png" width="600" height="500" style="float:right; ">

    </div>

   


</section>



<!--footer----->
<footer>
 <h4>© 2022 Jobs Portal. </h4>

<div class="foot">
    <a href="#">CONTACT US</a>
    <a href="#">LOGIN</a>
    <a href="#">SIGN UP</a>

</div>




</footer>
  
</body>
</html>